  ...
  <div className="max-w-6xl mx-auto">
    <ApartmentEstimator />
      ...
      <section className="px-6 py-12" style={{background: "linear-gradient(180deg, #f8fafc 0%, #f1f5f9 100%)"}}>
        <div className="max-w-6xl mx-auto">
          <h3 className="text-2xl font-bold text-gray-900 mb-2">{t.avgPrices}</h3>
          <p className="text-gray-500 mb-2 text-sm">{t.source}</p>
<div className="bg-amber-50 border border-amber-200 rounded-lg px-4 py-3 mb-6 text-sm text-amber-800">
  {lang === "ka" 
    ? "⚠️ ფასები ასახავს ახალი და მეორადი ბინების საშუალო მაჩვენებელს. ახალი მშენებლობები ჩვეულებრივ 20-30%-ით ძვირია."
    : "⚠️ Prices reflect averages of both new builds and resale apartments. New constructions are typically 20-30% higher."}
</div>
          <div className="flex flex-wrap items-center gap-2 mb-6">
            <span className="text-sm text-gray-500 font-medium">{t.showFor}</span>
            {FINISH_TYPES.map(f => (
              ...
              return (
                <div key={district.name} className="bg-white rounded-xl p-5 border border-gray-200 hover:shadow-md transition-shadow cursor-pointer">
                  <div className="flex justify-between items-start mb-3">
                    <div>
                      <h4 className="font-bold text-gray-900 text-lg">{district.nameKa}</h4>
                      <p className="text-gray-400 text-sm">{district.name}</p>
                    </div>
                    <span className={`text-xs px-2 py-1 rounded-full font-medium ${district.type === "premium" ? "bg-purple-100 text-purple-700" : district.type === "mid" ? "bg-blue-100 text-blue-700" : "bg-green-100 text-green-700"}`}>
                      {district.type === "premium" ? t.premium : district.type === "mid" ? t.mid : t.affordable}
                    </span>
                  </div>
                  <p className="text-3xl font-bold text-gray-900">
                    ${adjustedPrice.toLocaleString()}<span className="text-base font-normal text-gray-500">/კვ.მ</span>
                  </p>
                  ...
                  <p className={`text-sm mt-2 font-medium ${district.change > 0 ? "text-green-600" : district.change < 0 ? "text-red-500" : "text-gray-400"}`}>
                    {district.change > 0 ? "+" : ""}{district.change}% {t.yoy}
                  </p>
                </div>
              );
            })}
